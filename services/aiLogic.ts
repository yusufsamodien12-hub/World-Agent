...
// World-Agent's job is to DECIDE placement and grow the knowledge base --
// the actual 3D mesh for every object is delegated to BlockForge's /design
// tool (Mistral + PolyHaven-backed material/shape research). Every call is
// tagged with source: 'world-agent' so BlockForge can show a live "agent is
...
async function fetchCustomMeshFromBlockforge(description: string): Promise<CustomMeshSpec | undefined> {
  try {
    const resp = await fetch(BLOCKFORGE_DESIGN_URL, {
      ...
      body: JSON.stringify({ description, source: 'world-agent' }),
    });
    if (!resp.ok) {
...
export async function decideNextAction(
  ...
  try {
    ...
    // World-Agent focuses on placement + knowledge; BlockForge is always the
    // mesh source. Only fall back to the local hardcoded mesh if BlockForge
    // itself is unreachable, so the loop never stalls on a network hiccup.
